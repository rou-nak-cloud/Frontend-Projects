...
export default function AwardTaste() {
  ...
  useGSAP(
    () => {
      ...
      arrows.forEach((path) => {
        const length = path.getTotalLength(); // in PX
        gsap.set(path, {
          strokeDasharray: length, // set length to that PX
          // Create a solid dash that is exactly 300px long, followed by an invisible space that is 300px long." Because the dash is the exact same length as your entire arrow, the arrow still looks completely solid to the human eye.
          strokeDashoffset: length,
          // determines where the dash pattern begins. It lets you push or slide that dash pattern along the line. shifted the starting point by the exact length of the arrow (300px), you have completely pushed the solid 300px dash out of view. What is left sitting on your screen now is the 300px invisible empty space.
        });
      });
      tl.to(".arrow-1", {
        strokeDashoffset: 0, // push back to px length for view FIRST
        duration: 1.2,
        ease: "power1.inOut",
      });

      tl.to(
        // then SECOND animate with color
        ".arrow-1",
        {
          fill: "#0D1927",
          duration: 0.2,
  ...
  return (
    <>
      <section ref={tasteRef} className="text-center h-screen pt-8 pb-20">
        <h2 className="font-body text-4xl absolute top-30 left-15">
          Feel the Melt
        </h2>
